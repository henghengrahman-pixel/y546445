...
app.get('/documents/:type',auth,(req,res)=>{
  ...
  res.render('documents-list',{type,items:db.prepare(sql).all(...p),groups:db.prepare('SELECT * FROM groups_tbl WHERE active=1 ORDER BY name').all(),q,group,limit:threshold(type)});
});
