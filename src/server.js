...
function migrate() {
  ...
  if(!scols.includes('work_email')) db.exec("ALTER TABLE staff ADD COLUMN work_email TEXT");
...
function workDuration(startDate){if(!startDate)return '-';const start=new Date(`${startDate}T00:00:00`),now=new Date();let months=(now.getFullYear()-start.getFullYear())*12+(now.getMonth()-start.getMonth());if(now.getDate()<start.getDate())months--;months=Math.max(0,months);const y=Math.floor(months/12),m=months%12;const anchor=new Date(start);anchor.setMonth(anchor.getMonth()+months);const days=Math.max(0,Math.floor((now-anchor)/86400000));return `${y?y+' tahun ':''}${m?m+' bulan ':''}${days} hari`.trim();}
function expiryDuration(dateStr){if(!dateStr)return '-';const end=new Date(`${dateStr}T23:59:59`),now=new Date();let sign=end>=now?1:-1;let a=sign===1?now:end,b=sign===1?end:now;let months=(b.getFullYear()-a.getFullYear())*12+(b.getMonth()-a.getMonth());if(b.getDate()<a.getDate())months--;months=Math.max(0,months);const anchor=new Date(a);anchor.setMonth(anchor.getMonth()+months);const days=Math.max(0,Math.floor((b-anchor)/86400000));const y=Math.floor(months/12),m=months%12;const text=`${y?y+' tahun ':''}${m?m+' bulan ':''}${days} hari`.trim();return sign<0?`Lewat ${text}`:text;}
...
app.use((req,res,next)=>{
  ...
  res.locals.fmtDate=fmtDate; res.locals.daysUntil=daysUntil; res.locals.workDuration=workDuration; res.locals.expiryDuration=expiryDuration;
...
app.get('/groups/:id/staff',auth,(req,res)=>{
  const group=db.prepare('SELECT g.* FROM groups_tbl g WHERE g.id=?').get(req.params.id);
...
app.get('/groups',auth,(req,res)=>res.render('groups',{items:db.prepare(`SELECT g.*,COUNT(CASE WHEN s.employment_status='ACTIVE' THEN 1 END) staff_count FROM groups_tbl g LEFT JOIN staff s ON s.group_id=g.id GROUP BY g.id ORDER BY g.active DESC,g.name`).all()}));
app.post('/groups',auth,role('owner','admin'),(req,res)=>{try{const r=db.prepare('INSERT INTO groups_tbl(office_id,name,description) VALUES(NULL,?,?)').run(req.body.name,req.body.description);audit(req,'CREATE','group',r.lastInsertRowid,req.body.name);flash(req,'success','Group berhasil ditambahkan.');}catch(e){flash(req,'error',e.message);}res.redirect('/groups');});
...
app.post('/groups/:id/edit',auth,role('owner','admin'),(req,res)=>{
  db.prepare('UPDATE groups_tbl SET office_id=NULL,name=?,description=? WHERE id=?').run(req.body.name,req.body.description,req.params.id);
...
app.get('/staff/export.xlsx',auth,(req,res)=>{
  const q=(req.query.q||'').trim(),status=req.query.status||'ACTIVE',gender=req.query.gender||'',group=req.query.group||'',office=req.query.office||'',tenure=Number(req.query.tenure||0);
  let sql=`SELECT s.*,o.name office_name,g.name group_name,(SELECT COUNT(*) FROM warnings w WHERE w.staff_id=s.id) warning_count FROM staff s LEFT JOIN offices o ON o.id=s.office_id LEFT JOIN groups_tbl g ON g.id=s.group_id WHERE s.employment_status=?`;
  const params=[status];
  if(q){sql+=' AND (s.name LIKE ? OR s.staff_code LIKE ? OR s.position LIKE ? OR s.duty LIKE ? OR s.work_email LIKE ?)';params.push(...Array(5).fill(`%${q}%`));}
  if(gender){sql+=' AND s.gender=?';params.push(gender);}
  if(group){sql+=' AND s.group_id=?';params.push(group);}
  if(office){sql+=' AND s.office_id=?';params.push(office);}
  if(tenure){sql+=" AND s.start_date IS NOT NULL AND date(s.start_date, '+' || ? || ' months')<=date('now')";params.push(tenure);}
  sql+=' ORDER BY s.name COLLATE NOCASE';
  const rows=db.prepare(sql).all(...params).map((x,i)=>({
    No:i+1,'ID Staf':x.staff_code,Nama:x.name,Gender:x.gender==='FEMALE'?'Cewek':'Cowok',Kantor:x.office_name||'-',Group:x.group_name||'-','Tinggal di MES':x.shift||'-','Pegang / Tugas':x.duty||'-',Jabatan:x.position||'-','Email Kerja':x.work_email||'-','Mulai Kerja':x.start_date||'-','Lama Kerja':workDuration(x.start_date),'Jumlah SP':x.warning_count||0,Catatan:x.notes||'-'
  }));
  const ws=XLSX.utils.json_to_sheet(rows.length?rows:[{No:'',Keterangan:'Tidak ada data sesuai filter'}]);
  ws['!cols']=[{wch:6},{wch:14},{wch:28},{wch:10},{wch:20},{wch:20},{wch:20},{wch:20},{wch:18},{wch:28},{wch:14},{wch:20},{wch:12},{wch:30}];
  ws['!autofilter']={ref:ws['!ref']};
  const wb=XLSX.utils.book_new();XLSX.utils.book_append_sheet(wb,ws,'Data Staf');
  const buf=XLSX.write(wb,{type:'buffer',bookType:'xlsx'});
  audit(req,'EXPORT_EXCEL','staff',null,`Export ${rows.length} staf sesuai filter`);
  res.setHeader('Content-Type','application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
  res.setHeader('Content-Disposition',`attachment; filename=daftar-staf-${new Date().toISOString().slice(0,10)}.xlsx`);
  res.send(buf);
});
...
app.post('/staff',auth,role('owner','admin','hr'),(req,res)=>{
  ...
  try{
    ...
    const r=db.prepare(`INSERT INTO staff(staff_code,name,phone,telegram,office_id,group_id,shift,duty,position,start_date,notes,gender,contract_term,contract_end_date,work_email) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).run(
      sqlValue(code,''),
      sqlValue(req.body.name,''),
      sqlValue(req.body.phone),
      sqlValue(req.body.telegram),
      sqlValue(req.body.office_id),
      sqlValue(req.body.group_id),
      sqlValue(req.body.shift),
      sqlValue(req.body.duty),
      sqlValue(req.body.position),
      sqlValue(req.body.start_date),
      sqlValue(req.body.notes),
      sqlValue(req.body.gender,'MALE'),
      sqlValue(req.body.contract_term),
      sqlValue(contractEnd),
      sqlValue(req.body.work_email)
    );
  ...
  }catch(e){db.exec('ROLLBACK');console.error('[Staff Create]',e);flash(req,'error',e.message);res.redirect('/staff/new');}
});
...
app.post('/staff/:id',auth,role('owner','admin','hr'),(req,res)=>{const before=db.prepare('SELECT group_id FROM staff WHERE id=?').get(req.params.id);const contractEnd=req.body.contract_end_date||addContractDate(req.body.start_date,req.body.contract_term);db.prepare(`UPDATE staff SET name=?,phone=?,telegram=?,office_id=?,group_id=?,shift=?,duty=?,position=?,start_date=?,notes=?,gender=?,contract_term=?,contract_end_date=?,work_email=?,updated_at=CURRENT_TIMESTAMP WHERE id=?`).run(
  sqlValue(req.body.name,''),
  sqlValue(req.body.phone),
  sqlValue(req.body.telegram),
  sqlValue(req.body.office_id),
...
);audit(req,'UPDATE','staff',req.params.id,before?.group_id!=req.body.group_id?'Pindah group; seluruh dokumen tetap mengikuti staf':'Data staf diperbarui');flash(req,'success','Data staf diperbarui. Semua riwayat tetap mengikuti ID staf.');res.redirect(`/staff/${req.params.id}`);});
...
app.get('/warnings',auth,(req,res)=>{
  const q=(req.query.q||'').trim(),group=req.query.group||'',office=req.query.office||'';
  let sql=`SELECT s.id,s.staff_code,s.name,s.position,s.start_date,s.work_email,s.shift,o.name office_name,g.name group_name,COUNT(w.id) warning_count,MAX(w.warning_date) last_warning_date FROM staff s JOIN warnings w ON w.staff_id=s.id LEFT JOIN offices o ON o.id=s.office_id LEFT JOIN groups_tbl g ON g.id=s.group_id WHERE 1=1`;
  const p=[];if(q){sql+=' AND (s.name LIKE ? OR s.staff_code LIKE ?)';p.push(`%${q}%`,`%${q}%`);}if(group){sql+=' AND s.group_id=?';p.push(group);}if(office){sql+=' AND s.office_id=?';p.push(office);}sql+=' GROUP BY s.id ORDER BY date(last_warning_date) DESC,s.name';
  res.render('warnings-list',{items:db.prepare(sql).all(...p),q,group,office,groups:db.prepare('SELECT * FROM groups_tbl WHERE active=1 ORDER BY name').all(),offices:db.prepare('SELECT * FROM offices WHERE active=1 ORDER BY name').all()});
});
