...
const APP_NAME = envValue('APP_NAME', 'G - 8008 RP');
const DATA_DIR = envValue('DATA_DIR', path.join(__dirname, '..', 'data'));
const UPLOAD_DIR = path.join(DATA_DIR, 'uploads');
const DB_FILE = path.join(DATA_DIR, 'g8008rp.sqlite');
const IDLE_MS = Number(envValue('IDLE_TIMEOUT_MINUTES', '60')) * 60 * 1000;
const ABSOLUTE_MS = Number(envValue('ABSOLUTE_SESSION_HOURS', '12')) * 60 * 60 * 1000;
const TZ = envValue('TIMEZONE', 'Asia/Jakarta');
...
function seed() {
  const loginId = envValue('ADMIN_ID', 'ADM-0001');
  const password = envValue('ADMIN_PASSWORD', 'Admin123!');
  const name = envValue('ADMIN_NAME', 'Super Admin');
  const syncAdmin = envValue('SYNC_ADMIN_ON_BOOT', 'true').toLowerCase() !== 'false';
  const exists = db.prepare('SELECT id FROM users WHERE login_id=?').get(loginId);

  if (!exists) {
    db.prepare('INSERT INTO users(login_id,name,password_hash,role,active) VALUES(?,?,?,?,1)')
      .run(loginId, name, bcrypt.hashSync(password, 12), 'owner');
    console.log(`[Auth] Admin dibuat: ${loginId}`);
  } else if (syncAdmin) {
    db.prepare("UPDATE users SET name=?, password_hash=?, role='owner', active=1 WHERE id=?")
      .run(name, bcrypt.hashSync(password, 12), exists.id);
    console.log(`[Auth] Admin disinkronkan dari ENV: ${loginId}`);
  }
...
app.use(session({
  secret: envValue('SESSION_SECRET', 'change-this-session-secret-minimum-32-chars'),
  resave: false,
  saveUninitialized: false,
  rolling: true,
  cookie: { httpOnly: true, sameSite: 'lax', secure: envValue('NODE_ENV') === 'production', maxAge: IDLE_MS }
}));
...
app.get('/dashboard',auth,(req,res)=>{
  ...
  const expiring=db.prepare(`SELECT d.*,s.name,s.staff_code,o.name office_name,g.name group_name FROM documents d JOIN staff s ON s.id=d.staff_id LEFT JOIN offices o ON o.id=s.office_id LEFT JOIN groups_tbl g ON g.id=s.group_id WHERE date(d.expiry_date)>=date('now') ORDER BY date(d.expiry_date) LIMIT 12`).all();
  ...
  res.render('dashboard',{stats,expiring,upcomingLeave,notifications});
});
...
async function sendTelegram(text){const token=process.env.TELEGRAM_BOT_TOKEN,chat=process.env.TELEGRAM_CHAT_ID;if(!token||!chat)return;try{await fetch(`https://api.telegram.org/bot${token}/sendMessage`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({chat_id:chat,text,parse_mode:'Markdown'})});}catch(e){console.error('Telegram:',e.message);}}
