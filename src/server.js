...
app.get('/staff',auth,(req,res)=>{
  const q=(req.query.q||'').trim(); const status=req.query.status||'ACTIVE'; const gender=req.query.gender||''; const group=req.query.group||'';
  ...
  const params=[status];
  if(q){sql+=' AND (s.name LIKE ? OR s.staff_code LIKE ? OR s.duty LIKE ?)';params.push(`%${q}%`,`%${q}%`,`%${q}%`);}
  if(gender){sql+=' AND s.gender=?';params.push(gender);}
  if(group){sql+=' AND s.group_id=?';params.push(group);}
  sql+=' ORDER BY g.name,s.name';
  res.render('staff-list',{items:db.prepare(sql).all(...params),q,status,gender,group,groups:db.prepare('SELECT * FROM groups_tbl WHERE active=1 ORDER BY name').all()});
});
...
app.post('/staff',auth,role('owner','admin','hr'),(req,res)=>{
  ...
  try{
    ...
    const r=db.prepare(`INSERT INTO staff(staff_code,name,phone,telegram,office_id,group_id,shift,duty,position,start_date,notes,gender) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)`).run(code,req.body.name,req.body.phone,req.body.telegram,req.body.office_id||null,req.body.group_id||null,req.body.shift,req.body.duty,req.body.position,req.body.start_date||null,req.body.notes,req.body.gender||'MALE');
    const staffId=r.lastInsertRowid;
    const addDoc=(type,number,issued,expiry,notes='')=>{ if(expiry) db.prepare('INSERT INTO documents(staff_id,doc_type,doc_number,issued_date,expiry_date,notes) VALUES(?,?,?,?,?,?)').run(staffId,type,number||null,issued||null,expiry,notes||null); };
...
app.post('/staff/:id',auth,role('owner','admin','hr'),(req,res)=>{const before=db.prepare('SELECT group_id FROM staff WHERE id=?').get(req.params.id);db.prepare(`UPDATE staff SET name=?,phone=?,telegram=?,office_id=?,group_id=?,shift=?,duty=?,position=?,start_date=?,notes=?,gender=?,updated_at=CURRENT_TIMESTAMP WHERE id=?`).run(req.body.name,req.body.phone,req.body.telegram,req.body.office_id||null,req.body.group_id||null,req.body.shift,req.body.duty,req.body.position,req.body.start_date||null,req.body.notes,req.body.gender||'MALE',req.params.id);audit(req,'UPDATE','staff',req.params.id,before?.group_id!=req.body.group_id?'Pindah group; seluruh dokumen tetap mengikuti staf':'Data staf diperbarui');flash(req,'success','Data staf diperbarui. Paspor, Visa, dan WP otomatis tetap mengikuti staf ke group baru.');res.redirect(`/staff/${req.params.id}`);});
