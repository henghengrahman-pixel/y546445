...
app.post('/staff',auth,role('owner','admin','hr'),(req,res)=>{
  ...
  try{
    ...
    const r=db.prepare(`INSERT INTO staff(staff_code,name,phone,telegram,office_id,group_id,shift,duty,position,start_date,notes,gender,contract_term,contract_end_date,work_email) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).run(code,req.body.name,req.body.phone,req.body.telegram,req.body.office_id||null,req.body.group_id||null,req.body.shift,req.body.duty,req.body.position,req.body.start_date||null,req.body.notes,req.body.gender||'MALE',req.body.contract_term||null,contractEnd||null,req.body.work_email||null);
...
app.post('/staff/:id',auth,role('owner','admin','hr'),(req,res)=>{const before=db.prepare('SELECT group_id FROM staff WHERE id=?').get(req.params.id);const contractEnd=req.body.contract_end_date||addContractDate(req.body.start_date,req.body.contract_term);db.prepare(`UPDATE staff SET name=?,phone=?,telegram=?,office_id=?,group_id=?,shift=?,duty=?,position=?,start_date=?,notes=?,gender=?,contract_term=?,contract_end_date=?,work_email=?,updated_at=CURRENT_TIMESTAMP WHERE id=?`).run(req.body.name,req.body.phone,req.body.telegram,req.body.office_id||null,req.body.group_id||null,req.body.shift,req.body.duty,req.body.position,req.body.start_date||null,req.body.notes,req.body.gender||'MALE',req.body.contract_term||null,contractEnd||null,req.body.work_email||null,req.params.id);audit(req,'UPDATE','staff',req.params.id,before?.group_id!=req.body.group_id?'Pindah group; seluruh dokumen tetap mengikuti staf':'Data staf diperbarui');flash(req,'success','Data staf diperbarui. Semua riwayat tetap mengikuti ID staf.');res.redirect(`/staff/${req.params.id}`);});
