...
const APP_NAME = process.env.APP_NAME || 'G - 8008 RP';
...
function seed() {
  const loginId = process.env.ADMIN_ID || 'ADM-0001';
  const password = process.env.ADMIN_PASSWORD || 'Admin123!';
  const name = process.env.ADMIN_NAME || 'Super Admin';
  const exists = db.prepare('SELECT id FROM users WHERE login_id=?').get(loginId);
  if (!exists) db.prepare('INSERT INTO users(login_id,name,password_hash,role) VALUES(?,?,?,?)').run(loginId, name, bcrypt.hashSync(password, 12), 'owner');
...
app.use(session({
  secret: process.env.SESSION_SECRET || 'change-this-session-secret-minimum-32-chars',
  resave: false,
  saveUninitialized: false,
  rolling: true,
  cookie: { httpOnly: true, sameSite: 'lax', secure: process.env.NODE_ENV === 'production', maxAge: IDLE_MS }
}));
