...
app.post('/staff/:id/documents',auth,role('owner','admin','hr'),upload.single('file'),(req,res)=>{const r=db.prepare('INSERT INTO documents(staff_id,doc_type,doc_number,issued_date,expiry_date,file_path,notes) VALUES(?,?,?,?,?,?,?)').run(req.params.id,req.body.doc_type,req.body.doc_number,req.body.issued_date||null,req.body.expiry_date,req.file?`/uploads/${req.file.filename}`:null,req.body.notes);audit(req,'CREATE','document',r.lastInsertRowid,req.body.doc_type);flash(req,'success','Dokumen berhasil disimpan.');res.redirect(`/staff/${req.params.id}`);setImmediate(()=>generateExpiryNotifications([Number(r.lastInsertRowid)]));});
